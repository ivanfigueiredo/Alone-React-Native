...
export const infoUser = async () => {
  ...
  const req = await fetch('https://backend-simodis.herokuapp.com/user/info?token='+`${token}`);
...
export const myCourse = async () => {
  const token = await myToken();
  const req = await fetch('https://backend-simodis.herokuapp.com/course/mycourse?token='+`${token}`);
